...
def calculate_radius_at_layer(layer: float) -> float:
    if isinstance(layer, np.ndarray):
        angle = np.where(
            ...
            (np.pi / 2)
            + ((layer - PEAK_LAYER) / (TOTAL_LAYERS - PEAK_LAYER)) * (np.pi / 2),
        )
    else:
        if layer <= PEAK_LAYER:
            ...
            angle = (np.pi / 2) + (
                (layer - PEAK_LAYER) / (TOTAL_LAYERS - PEAK_LAYER)
            ) * (np.pi / 2)
...
def create_continuous_wave_wall() -> gc.Path:
    ...
    z = np.linspace(
        (BOTTOM_LAYERS + 1) * LAYER_HEIGHT,
        (BOTTOM_LAYERS + 1 + wall_layers) * LAYER_HEIGHT,
        total_points,
    )
